...
class UserController extends Controller
{
    public function index($username)
    {
        ...
        $messs = Message::where('idUserRecive', Auth::id())->get();
        $messSend = Message::where('idUserSend', Auth::id())->first();
        // dd($messSend);
        return view('user.index', ['user' => $user, 'messs' => $messs, 'messSend' => $messSend]);
    }
            ...
            if ($request->change_pass) {
                ...
                if ($request->username != $user->username) {
                    $check = request()->validate([
                        'name' => ['required', 'string', 'max:20', 'min:3', 'unique:users', new Username()]
                    ]);
    ...
    public function sendmess(Request $request, $user)
    {
        try {
            ...
            $mess = Message::where('idUserSend', Auth::id())->first();
            if ($mess) {
                $mess->update([
                    'idUserSend' => Auth::id(),
                    'idUserRecive' => $user,
