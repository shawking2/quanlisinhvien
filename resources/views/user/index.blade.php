...
@section('content')
        ...
        <div class="row pb-3 ms-1">
            <div class="card-header bg-light ">Tin nhắn</div>
            <div class="card-body">
               @each('layouts.home.list-mess', $messs, 'mess')
            </div>
        @else
    ...
    <div class="form-row pt-3 pb-2 ps-3 w-100">
        {{ csrf_field() }}
            <div class="row pb-3 ms-1">
                <div class="form-outline col-md-4">
                    <input type="text" nc-notnul="Tên Thể loại không được bỏ trống"
                        class="form-control @error('content') is-invalid @enderror mb-0" name="content" id="content" value="{{$messSend ? $messSend->content : "" }}"
